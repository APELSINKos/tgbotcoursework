...
stateDiagram-v2
    ...
    state "Ожидание ввода" as Step1
    state "Ожидание следующего ввода" as Step2

    [*] --> Menu
    Menu --> Step1: кнопка действия
    Step1 --> Step1: ошибка ввода
    Step1 --> Step2: далее
    Step2 --> Step2: ошибка ввода
    Step1 --> Menu: сохранено или отмена
    Step2 --> Menu: сохранено или отмена
```

«Кнопка действия» — это «➕ Добавить», «🗑 Удалить», «🏙 Сменить город», «🕗 Время сводки» или одна из кнопок конвертера. Второй шаг есть только у напоминаний: сначала текст, потом время.
